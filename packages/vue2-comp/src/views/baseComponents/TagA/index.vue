  ...
  <div class="modal">
    ...
    <div class="modal-content">
      <div class="detail-before detail">
        <div class="title">原来的效果 都是黑色 点击也不会变灰</div>
      ...
      <div class="detail-now detail">
        <div class="title">现在的效果 就是实际需求效果</div>

        <div v-for="item in list" :key="item.id">
          <router-link
            :class="['list-link', item.create_time.startsWith(today) ? 'is-new': '']"
            :to="getTargetRoute(item)"
          >
            <div :title="item.title" class="list">
              <div class="list-title">{{ item.title}}</div>
              <div class="list-date">{{ item.create_time }}</div>
            </div>
          </router-link>
        </div>
